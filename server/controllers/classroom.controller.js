...
export const uploadStudyMaterial = async (req, res) => {
  try {
    ...
    if (!name || !resolvedMimeType || !url) {
      return res.status(400).json({ error: "name, mimeType and url are required" });
    }
    ...
    subject.uploads.push({
      ...
      url: String(url).trim(),
      downloadUrl: downloadUrl ? String(downloadUrl).trim() : undefined,
      path: path ? String(path).trim() : undefined,
      cloudinary: cloudinary || (publicId || version || resourceType || format || secureUrl ? {
        publicId: publicId ? String(publicId).trim() : "",
        version: Number(version || 0) || null,
        resourceType: resourceType ? String(resourceType).trim() : "",
        format: format ? String(format).trim() : "",
        secureUrl: secureUrl ? String(secureUrl).trim() : "",
      } : undefined),
      publicId: publicId ? String(publicId).trim() : undefined,
      version: Number(version || 0) || undefined,
      resourceType: resourceType ? String(resourceType).trim() : undefined,
      format: format ? String(format).trim() : undefined,
      secureUrl: secureUrl ? String(secureUrl).trim() : undefined,
    ...
    try {
      ...
      res.json({
        ...
        upload: {
          ...
          secureUrl: upload.secureUrl || null,
