export default function NotificationsWidget({
    ...
    return (
        <div className="premium-card min-h-[138px] min-[426px]:min-h-[150px] min-[769px]:min-h-[180px] p-4 min-[769px]:p-6 flex-1 flex flex-col items-center justify-center gap-2 relative">
            <h3 className="text-[11px] min-[426px]:text-xs min-[769px]:text-xs font-black text-gray-500 uppercase tracking-widest w-full text-center z-10 mb-1">
                Notifications
            </h3>
                    ...
                    {activeItem ? (
                        ...
                        >
                            <div key={activeNotif} className="animate-fade-in-up">
                                <p className={`text-[11px] min-[426px]:text-xs min-[769px]:text-xs font-bold uppercase mb-1 ${activePalette.text}`}>
                                    {String(activeItem?.type || "info").replace('_', ' ')}
                                </p>
                                <p className="text-[15px] min-[426px]:text-[15px] min-[769px]:text-[15px] font-bold text-gray-800 line-clamp-2">{activeItem?.title || "Notification"}</p>
                                <p className="text-[11px] min-[426px]:text-xs text-gray-500 line-clamp-2 mt-0.5">{activeItem?.desc || ""}</p>
                            </div>
                        </div>
                    ) : (
                        <p className="pl-2 text-[11px] min-[426px]:text-xs text-gray-400 font-medium">No notifications yet.</p>
                    )}
                </div>
            </div>
        </div>
