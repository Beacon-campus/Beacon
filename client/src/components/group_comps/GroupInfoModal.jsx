...
export default function GroupInfoModal({ isOpen, onClose, group }) {
  ...
  return (
    <Modal 
      ...
      className="max-[768px]:max-w-full max-[768px]:h-[100dvh] max-[768px]:max-h-[100dvh] max-[768px]:!rounded-none min-[769px]:max-w-md w-full bg-white min-[769px]:!rounded-[30px] overflow-hidden flex flex-col min-[769px]:max-h-[90vh]"
    >
      <div className="h-24 w-full bg-indigo-50 relative shrink-0">
        <button 
          onClick={onClose} 
          className="absolute top-4 left-4 z-10 w-10 h-10 bg-black/5 hover:bg-black/10 backdrop-blur-md rounded-full flex items-center justify-center text-indigo-900 min-[769px]:hidden transition-colors"
        >
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 soft-scrollbar flex flex-col">
